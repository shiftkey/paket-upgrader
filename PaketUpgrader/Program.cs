...
namespace PaketUpgrader
{
    ...
    class Program
    {
        ...
        static async Task Run()
        {
            ...
            await upgrader.Run("SAFE-Stack/SAFE-BookStore");
            await upgrader.Run("FsProjects/Argu");
            await upgrader.Run("TeaDrivenDev/StatusPotentiae");
        }
    }
    ...
    public class PaketUpgrader
    {
        ...
        public async Task Run(string ownerAndName)
        {
        ...
        private async Task PerformUpgrade(string owner, string name)
        {
...
The work to update Paket in the wild is occurring here: https://github.com/fsprojects/Paket/issues/3068";
        ...
        private async Task<PullRequest> HasOpenPullRequest(string owner, string name)
        {
            ...
            foreach (var pullRequest in pullRequests)
            {
                ...
                var touchesKeyFiles = files.FirstOrDefault(f => f.FileName == ".paket/paket.exe" || f.FileName == ".paket/paket.bootstrapper.exe");

                if (touchesKeyFiles != null)
                {
                    return pullRequest;
                }
            }
        ...
        private async Task<Repository> FindRepositoryToSubmitPullRequestFrom(string owner, string name)
        {
            var repository = await client.Repository.Get(owner, name);
        ...
        private async Task<string> GetNewExecutableBase64()
        {
        ...
        private async Task<Reference> CreateNewReferenceWithPatch(Repository repository)
        {
